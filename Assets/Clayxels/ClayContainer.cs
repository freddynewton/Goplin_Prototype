...
namespace Clayxels{
	...
	*/
	...
	public class ClayContainer : MonoBehaviour{
		...
		/* An interactive container can be changed while the game is running, 
		...
		*/
		...
		static public void initGlobalData(){
			...
			ClayContainer.lastEditedContainerId = -1;
		...
		/* Automatically invoked once when the game starts, you might need to invoke this yourself for procedurally built scenes.*/
		public void init(){
			if(!this.needsInit){
				return;
			}
			...
			this.chunkSize = this.clayDetailToChunkSize();//(int)Mathf.Lerp(40.0f, 4.0f, (float)this.clayxelDetail / 100.0f);
			this.limitChunkValues();
			...
			// if(this.autoBounds){
			// 	bool forceInit = true;
			// 	this.updateAutoBounds(forceInit);
			// }
			...
			#if UNITY_EDITOR
				if(!Application.isPlaying){
					if(this.checkEditorSelectingThisContainer()){
						this.editingThisContainer = true;
						ClayContainer.lastEditedContainerId = this.containerId;
					}
				}
			#endif
		...
		public void computeClay(){
			...
			if(this.memoryOptimized){
				...
				if(ClayContainer.lastEditedContainerId != this.containerId){
					this.switchEditedContainer();
				}

				if(this.memoryOptimized){// check if memory still optimized if switch of edited container did not happen
					this.expandMemory();
				}
			}
		...
		*/
		...
		public void freezeToMesh(int meshDetail){
			...
			Material mat = null;
				...
				string assetPath = "Assets/" + ClayContainer.defaultAssetsPath + "/" + assetNameUnique + ".mat";
				
				mat = (Material)AssetDatabase.LoadAssetAtPath(assetPath, typeof(Material));
			...
			if(mat == null){
				...
				this.transferMaterialPropertiesToMesh();
			}
		...
		public void defrostToLiveClayxels(){
			...
			if(this.gameObject.GetComponent<MeshRenderer>() != null){
				...
				// this.gameObject.GetComponent<MeshRenderer>().material = null;// fix flicker on microvoxel renderer
				this.gameObject.GetComponent<MeshRenderer>().sharedMaterial = null;
			}

			this.enableAllClayObjects(true);
		}

		/* ClayContainers that have interactive set to false will disable all their clayObjects when the game starts.
		...
		public void freezeContainersHierarchyToMesh(){
			...
			for(int i = 0; i < containers.Length; ++i){
				...
				container.freezeToMesh(container.clayxelDetail);
			}
		}
		...
		*/
		...
		/* Set the quality of microvoxel splats, 
		...
		*/
		...
		/* Set how far a ray will travel inside a microvoxel container.
		...
		*/
		...
		/* Set if the camera can go inside this container, 
		...
		*/
		...
		static int lastEditedContainerId = -1;
		...
		static void scanInstances(){
			...
			for(int i = 0; i < containers.Length; ++i){
				...
				if(container.instanceOf != null){
					if(!container.instanceOf.instances.Contains(container)){
		...
		void initInstancesData(){
			...
			if(this.autoBounds){
				...
				if(this.renderMode == ClayContainer.RenderModes.smoothMesh){
					numChunks = 1;
				}
			}
		...
		void drawClayxelsMicroVoxel(){
			...
			// this.materialProperties.SetBuffer("splatTexIdGlob", this.splatTexIdBuffer);
		...
		void computeClaySmoothMesh(){
			...
			ClayContainer.claycoreCompute.SetBuffer((int)Kernels.computeMeshRealTime, "indirectDrawArgs", this.indirectDrawArgsBuffer);
			...
			ClayContainer.claycoreCompute.SetBuffer((int)Kernels.computeMeshRealTime2, "indirectDrawArgs", this.indirectDrawArgsBuffer);
		...
		void transferMaterialPropertiesToMesh(){
			MeshRenderer render = this.gameObject.GetComponent<MeshRenderer>();
			if(render == null){
				return;
			}
		...
		void optimizeMemorySmoothMesh(){
			...
			if(ClayContainer.tmpStorageVec3.Length < numElements){
				ClayContainer.tmpStorageVec3 = new Vector3[numElements];
			}

			if(ClayContainer.tmpStorageVec4.Length < numElements){
				ClayContainer.tmpStorageVec4 = new Vector4[numElements];
			}
			...
			this.smoothMeshPointsBuffer.GetData(ClayContainer.tmpStorageVec3, 0, 0, numElements);
			this.smoothMeshPointsBuffer.Release();
			this.compBuffers.Remove(this.smoothMeshPointsBuffer);
			this.smoothMeshPointsBuffer = new ComputeBuffer(numElements, sizeof(float) * 3);
			this.smoothMeshPointsBuffer.SetData(ClayContainer.tmpStorageVec3, 0, 0, numElements);
			this.compBuffers.Add(this.smoothMeshPointsBuffer);

			this.smoothMeshNormalsBuffer.GetData(ClayContainer.tmpStorageVec4, 0, 0, numElements);
			this.smoothMeshNormalsBuffer.Release();
			this.compBuffers.Remove(this.smoothMeshNormalsBuffer);
			this.smoothMeshNormalsBuffer = new ComputeBuffer(numElements, sizeof(float) * 4);
			this.smoothMeshNormalsBuffer.SetData(ClayContainer.tmpStorageVec4, 0, 0, numElements);
		...
		void optimizeMemoryMicrovoxels(){
			...
			if(this.gridPointersMip3Buffer != ClayContainer.gridPointersMip3GlobBuffer){
				this.compBuffers.Remove(this.gridPointersMip3Buffer);
				this.gridPointersMip3Buffer.Release();
			}

			if(this.gridPointersMip2Buffer != ClayContainer.gridPointersMip2GlobBuffer){
				this.compBuffers.Remove(this.gridPointersMip2Buffer);
				this.gridPointersMip2Buffer.Release();
			}

			if(this.pointCloudDataMip3Buffer != ClayContainer.pointCloudDataMip3GlobBuffer){
		...
		void expandMemoryMicroVoxel(){
			if(ClayContainer.containersInScene.ContainsKey(ClayContainer.lastEditedContainerId) && ClayContainer.lastEditedContainerId != this.containerId){
				ClayContainer lastEditedContainer = ClayContainer.containersInScene[ClayContainer.lastEditedContainerId];
				if(lastEditedContainer != null){
					lastEditedContainer.editingThisContainer = false;

					if(!lastEditedContainer.interactive && !lastEditedContainer.memoryOptimized){
						lastEditedContainer.optimizeMemory();
					}
				}
			}
			...
			if(this.gridPointersMip3Buffer != null){
				this.gridPointersMip3Buffer.Release();
				this.gridPointersMip2Buffer.Release();
				this.pointCloudDataMip3Buffer.Release();
			}
		...
		void initMaterialProperties(){
			...
			if(this.customMaterial != null){
            	...
            	bool newSRP = ClayContainer.isNewSRP();

				if(ClayContainer.renderPipe == "hdrp" && newSRP){
            		renderModeMatSuffix += "_2020_2";
	            }
				...
				if(ClayContainer.renderPipe == "urp" && newSRP){
		...
		void updateAutoBounds(bool forceInit = false){
			...
			int detailChunkSize = this.clayDetailToChunkSize();//(int)Mathf.Lerp(40.0f, 4.0f, (float)this.clayxelDetail / 100.0f);
		...
		static void onPrefabUpdate(GameObject obj){
			...
			if(obj.GetComponentsInChildren<ClayContainer>().Length == 0){
		...
		void onSelectionChanged(){
			...
			if(!this.editingThisContainer){
				...
				if(wasEditingThis){// we're changing selection
					if(!this.interactive){
						this.optimizeMemory();
